...
export const ProgressSlider = ({
  ...
  return (
    ...
    >
      <div className={cn("relative font-dmsans", className)}>{children}</div>
    </ProgressSliderContext.Provider>
  );
};

export const SliderContent = ({ children, className }) => {
  return <div className={cn("font-dmsans", className)}>{children}</div>;
};
    ...
    <AnimatePresence mode="popLayout">
      {active === value && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={cn("font-dmsans", className)}
        >
          {children}
        </motion.div>
      )}
...
export const SliderBtn = ({ children, value, className, progressBarClass }) => {
  ...
  return (
    <button
      className={cn(
        `relative font-dmsans ${active === value ? "opacity-100" : "opacity-50"}`,
        className,
      )}
    ...
    >
      {children}
      <div
        className="absolute font-dmsans inset-0 overflow-hidden -z-10 max-h-full max-w-full "
        role="progressbar"
        aria-valuenow={active === value ? progress : 0}
      >
        <span
          className={cn("absolute left-0 font-dmsans ", progressBarClass)}
          style={{
